...
const timeIntervalFormSchema = z.object({
  ...
  .refine(intervals => {
    ...
    message: 'O horário de término precisa ser pelo menos 1h distante do inicio.'
  })
  ,
})
...
export default function TimeIntervals() {
  ...
  return (
    <>
      <NextSeo
        title="Selecione sua disponibilidade | Calendar"
        noindex
      />
      <Container>
        <Header>
          ...
          <IntervalContainer>
            {fields.map((field, index) => (
              <IntervalItem key={field.id}>
                ...
                <IntervalInputs>
                  <TextInput size="sm" type="time" disabled={intervals[index].enabled === false} step={60} {...register(`intervals.${index}.startTime`)} />
                  <TextInput size="sm" type="time" disabled={intervals[index].enabled === false} step={60} {...register(`intervals.${index}.endTime`)} />
                </IntervalInputs>
              </IntervalItem>
            ))}
          </IntervalContainer>
